...
function encodeSync(inputData: InputData, opts?: Options): ArrayBuffer {
  opts = opts || {};

  const audioData = toAudioData(inputData);
  ...
  const bitDepth = floatingPoint ? 32 : ((opts.bitDepth === undefined ? 0: opts.bitDepth) || 16);
...
function encode(inputData: InputData, opts: Options): Promise<ArrayBuffer> {
  return new Promise(function(resolve) {
    resolve(encodeSync(inputData, opts));
  });
}

function toAudioData(inputData: InputData): AudioData | null {

  if (typeof inputData.sampleRate !== "number") {
    return null;
  }
  if (!Array.isArray(inputData.channelData)) {
    return null;
  }
  if (!(inputData.channelData[0] instanceof Float32Array)) {
    return null;
  }

  const audioData = {
    numberOfChannels: inputData.channelData.length,
    length: inputData.channelData[0].length|0,
    sampleRate: inputData.sampleRate|0,
    channelData: inputData.channelData
  };
...
function writeData(writer: Writer, format: Format, length: number, audioData: AudioData, opts: Options): void | TypeError {
  const bitDepth = format.bitDepth;
  const encoderOption = format.floatingPoint ? "f" : opts.symmetric ? "s" : "";
  const methodName = ("pcm" + bitDepth + encoderOption) as keyof Writer;

  if (!writer[methodName]) {
    return new TypeError("Not supported bit depth: " + bitDepth);
  }

  const write = writer[methodName].bind(writer) as (arg: number) => void;
...
function createWriter(dataView: DataView): Writer {
  let pos = 0;
